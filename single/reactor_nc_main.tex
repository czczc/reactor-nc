...
role in the study of \st{the} neutrino oscillations, a phenomenon that indicates that neutrinos have mass and that the neutrino flavors are quantum mechanical mixtures. Over the past several decades reactors were used in the discovery of neutrinos, were crucial in solving the solar neutrino puzzle, and allowed the determination of the smallest mixing angle $\theta_{13}$. In the near future, reactors will help to determine the neutrino mass hierarchy and to solve the puzzling issue of sterile neutrinos.
\end{abstract}
...
\label{sec:intro}
Neutrinos, the products of  radioactive decay among other things, are somewhat enigmatic, since 
\textcolor{blue}{\st{they interact very weakly with anything}}. 
they can travel enormous distances through matter without interacting even once. 
\st{Hence they are very difficult to observe.} 
Understanding their properties in detail is fundamentally important, \textcolor{blue}{ \st{a}} and notwithstanding that they are so very difficult to observe,
great progress in this field has been achieved in 
\textcolor{blue}{\st{the}} recent decades.   
...
about \st{the} elementary particles and their interactions. \textcolor{blue}{ Nevertheless, the Standard Model is incomplete in view of most physicists.}

The Standard Model of electroweak interactions,
developed in late 1960s, incorporates neutrinos \textcolor{red}{($\nu_e$, $\nu_\mu$, $\nu_\tau$)} as left-handed partners of the \textcolor{red}{three families of} charged leptons \textcolor{red}{($e^-$, $\mu^-$, $\tau^-$)}. Since \st{the} weak interactions 
are the
only way neutrinos  interact with anything, the unneeded right-handed components of the neutrino field are absent 
\textcolor{blue}{in the Model} by definition and neutrinos are assumed to be massless with the individual lepton numbers, \textcolor{blue}{i.e. the number of leptons of the
...
despite the long \st{tradition of efforts to} search for them.

The discovery of neutrino oscillations over the past several decades
proved that these assumptions were incorrect. \textcolor{blue}{That discovery \st{ it}}
represents one of the very few instances that show that 
the \textcolor{blue}{ \st{otherwise extremely successful}}  Standard Model is \textcolor{blue}{indeed} incomplete. 
\textcolor{blue}{The phenomenon of neutrino oscillations} means that
neutrinos have a finite mass, albeit very small, and that lepton flavor is not a conserved quantity.
Box 1 explains the basic physics of neutrino oscillations and their relation with neutrino masses. \textcolor{blue}{The boxed text}
...
in the latest editions of the Review of Particle Physics~\cite{PDG14} \textcolor{blue}{and is also shown in Box 1}.
...
used as an example, is plotted as a function of the distance to the reactor cores. Reactor experiments are placed at different baselines \st{in order} 
...
distribution in \st{the} nuclear beta decay. 
However, \textcolor{blue}{\st{due to their weakly-interacting nature,} it was not until \st{only}}  1953--1959 that Reines and Cowan~\cite{Reines53,Cowan56,Reines59} were able to show that neutrinos were real particles. 
...
\textcolor{red}{These pioneering short-baseline experiments did not observe variations with distance, as predicted by the later established three-neutrino oscillation theory, 
but they were important for the understanding of the reactor neutrino flux and spectrum.}
The KamLAND experiment~\cite{Kamland03,Kamland05,Kamland08} in 2000s \st{has} convincingly showed that the 
earlier solar neutrino measurements were indeed caused by oscillations. 
It demonstrated for the first time that the reactor neutrinos indeed oscillate, i.e. that the $\bar{\nu}_e$ component changes 
\textcolor{blue}{\st{as predicted}} with $L/E_{\nu}$ \textcolor{red}{, as explained in Box 1.}
...
value of the mixing angle $\theta_{13}$ was successfully resolved; it turns out that its value $\theta_{13} \sim 8.4^\circ$ \textcolor{blue}{(or $\sin^2 2\theta_{13}$ = 0.095)}
...
\textcolor{blue}{\st{allow}let} us determine the so-far missing fundamental features of the oscillations, the neutrino mass hierarchy, and the phase $\delta_{CP}$ that characterizes the possible CP (charge and parity) violation. 
...
\st{Finally,} Most of the oscillation results are well described by the simple three-neutrino generations hypothesis. However, there are a few anomalous indications,
...
fourth, or fifth, etc. \st{more} neutrino families called sterile neutrinos. These lack weak interactions and \textcolor{blue}{\st{are}
would be} observable only \textcolor{blue}{\st{due to their}when} mixing with the familiar active neutrinos.  The proposed very short-baseline reactor experiments at distances $\sim$10 m \textcolor{red}{\st{might decide} will test} whether this fascinating possibility is realistic or not.
...
Nuclear reactors derive their power from fission. The fission fragments are neutron rich and undergo a cascade of $\beta$ decays. Each \st{act of} fission
...
$\sim$ 200 MeV of energy in each \st{act of} fission, the typical yield of $\bar{\nu}_e$ at equilibrium is $\sim 6 \times 10^{20} \bar{\nu}_e$ core$^{-1}$ s$^{-1}$.   
...
muon (mass 105.7 MeV) or tau (mass 1776.8 MeV); \textcolor{blue}{with} neutrino energy \st{must be} sufficient to
...
\textcolor{blue}{To reduce the dependence on the
knowledge of the reactor spectrum more recent experiments
...
There are two principal and complementary ways to evaluate antineutrino spectra associated with fission. 
 ...
 with the experimentally known (or theoretically deduced) branching ratios $b_{n,i}(E^i_0)$ of all $\beta$-decay branches with \st{the} endpoints $E^i_0$ and a
 ...
 \end{equation}
 There are several difficulties with this method. The branching ratios and endpoint energies are sometimes poorly known 
 \textcolor{blue}{\st{(or not at all)} at best}, in particular for
...
\textcolor{blue}{ with very large neutron capture cross section }\textcolor{red}{ such as gadolinium).} The detection of this prompt-delayed signal pair indicates an $\bar{\nu}_e$ candidate.}
 \end{figure}
 ...
 trivial.}}
 \textcolor{blue}{The conversion to the $\bar{\nu}_e$ spectrum is performed in each of these virtual branches. That conversion is based on the assumption 
 that the electron spectrum is known precisely.}
 When \textcolor{blue}{\st{putting} all virtual branches are put} together one has \textcolor{blue}{also} take into account \st{the fact} that different nuclear charges $Z$ 
 ...
 While the \st{procedure} conversion would introduce only minimum uncertainty if all decays would be of the allowed shape, the presence
 of the first forbidden decays \st{again} introduces \textcolor{blue}{additional} uncertainty whose magnitude is difficult to determine accurately.

The summation method was used
initially in~\cite{Davis,Vogel81,Klapdor-Pu,Klapdor-U,Kopeikin} and in the \st{updated} more recent version in~\cite{Mueller}\textcolor{red}{\st{[Huber]}}. 
\textcolor{red}{\st{The conversion method is based on a series
...
Naturally, the thermal power of the reactor and its time-changing fuel composition must be known, as  \textcolor{blue}{\st{well} must}  the 
energy associated with fissions of the isotopes $^{235}$U, $^{239}$Pu, $^{241}$Pu
and $^{238}$U. In addition, \textcolor{blue}{as already mentioned,} small corrections to the spectrum shape of individual $\beta$-decay branches due to the radiative correction, weak magnetism,
nuclear size,
...
first forbidden decays are included it is estimated in \st{the} Ref.~\cite{Hayes} that the uncertainty increases to $\sim$ 5\%.
...
positrons and the delayed neutron captures is a powerful tool for \st{the} background suppression. Note that the reaction (\ref{eq:detection}) has
...
to the reactor power are also shown. \textcolor{blue}{In the top part} the steps involved in the $\bar{\nu}_e$ capture on proton reaction are schematically indicated. 
...
Neutrinos are produced with a definite `flavor' \textcolor{blue}{$\nu_e, \nu_{\mu}, \nu_{\tau}$}. For example, in nuclear $\beta$ decay electron
...
mass, the flavor composition of a neutrino beam could \st{be modified; it can} vary regularly as
...
Let us assume that there are \textcolor{blue}{only} two massive neutrinos $\nu_i, i=1,2$ with different masses $m_i$.
\textcolor{blue}{In quantum mechanics the development of a state $\psi$ with momentum $p$  is governed
by $\psi(t) = \psi(0) e^{ipL}$. (We use the usual system of units where $\hbar = c =1$). For highly relativistic neutrinos
$p = \sqrt{E^2 - m^2} \sim E - m^2/2E$. Thus, when neutrinos \st {When they}} propagate in vacuum over a distance $L$,
each acquires the phase $\nu_i(L) = \nu_i (0) \exp(-i m^2_i L/2E)$. (The overall phase is skipped
\textcolor{blue}{\st{and it is assumed that the neutrinos are highly relativistic,
$p \sim E - m^2/2E$}}. Additional phases are acquired when neutrinos propagate in \st{the} matter, the so-called \st{the} 
...
\end{equation}
In order to observe this beam at $L$ we must use the weak interactions.  We must \textcolor{blue}{therefore} project the $\nu_i$ back to the flavor basis $\nu_e$ and $\nu_{\alpha}$.
Thus
\begin{equation}
...
of detecting $\nu_{\alpha}$ is the \st{corresponding} square of the $\nu_{\alpha}(0)$ amplitude. \st{Therefore,} After a bit of a simple algebra
\textcolor{blue}{this becomes:}
...
Observation of neutrino oscillations, \textcolor{blue}{\st{therefore} consequently}, constitutes a proof that at least some of the neutrinos have a finite mass and that the superposition is
...
In 2001, the SNO experiment~\cite{SNO}, \textcolor{red}{for the first time,} successfully measured the total flux of all three neutrino flavors through the neutral current channel $\nu + d \to \nu + p + n$ using heavy water as a target, \textcolor{blue}{\st{and} that} yielded 
\textcolor{blue}{results} consistent with the SSM. 
The SNO result is considered \textcolor{blue}{\st{as}} the ``smoking gun'' evidence of the neutrino oscillation explanation to the Solar Neutrino Problem --- The solar neutrinos, produced as electron-neutrinos from fusion
\textcolor{blue}{and other reactions in the central region of \st{ inside}} the Sun, \textcolor{blue}{\st{have} are} transformed into other flavors when they arrived at the Earth.
...
\begin{figure*}[htb]
  ...
  \textcolor{blue}{The curve deviates from the perfect sinusoidal $L/E$ dependence since KamLAND has multiple baselines.}
  % \textcolor{red}{(since KamLAND has multiple baselines it deviates from Eqn.~\ref{eq:osci}).}
  }
\end{figure*}

The solar neutrino experiments \textcolor{blue}{\st{left} allowed} several possible solutions in the oscillation parameter space of $\theta_{12}$ and $\Delta m^2_{21}$. 
A precise measurement of these parameters and demonstration of the oscillatory feature, however, \textcolor{blue}{\st{is} was} 
hindered by the relatively large uncertainty of the solar $\nu_{e}$ flux predicted by the SSM, the large matter effect inside the Sun, and the extremely long distance the neutrinos travel. 
...
The reactor operation information such as thermal power, fuel burn-up, and fuel exchange and enrichment records 
\textcolor{blue}{\st{are} were} provided by all Japanese reactors, which allow\textcolor{blue}{ed} KamLAND to calculate the instantaneous fission rate of each isotope accurately. 
...
The other dominant background ($\sim$10\%) at KamLAND is caused by the $^{13}$C$(\alpha,n)^{16}$O reaction where the $\alpha$-decay comes from $^{210}$Po, \textcolor{red}{a decay product of $^{222}$Rn that is naturally present in the air and many materials as traces, but is \textcolor{blue}{sufficient} to induce a measurable 
contamination of the scintillator during its production.}
The neutron scattering off proton or $^{16}$O$^*$ de-excitation produces a prompt signal, followed by a neutron capture delayed signal. This then mimics a true $\bar\nu_e$ event. The rest of the backgrounds include: the antineutrinos produced in the decay chains of $^{232}$Th and $^{238}$U in the Earth's interior,
\textcolor{blue}{mantle, and crust} (geoneutrinos); the cosmogenic beta-delayed neutron emitters $^{9}$Li and $^{8}$He;  the fast neutrons from muons passing through the surrounding rock, as well as atmospheric neutrinos. 

Fig.~\ref{fig:kamland} (left) shows the prompt energy spectrum of $\bar\nu_e$ candidate
events, observed with 2.9 kton$\cdot$year exposure, overlaid with the expected reactor $\bar\nu_{e}$ and background spectra. A total of 1609 events were observed, which is only about 60\% of the expected signal if there \textcolor{blue}{\st{is} are} no oscillations. The ratio of the background-subtracted $\bar\nu_e$ candidate events to no-oscillation expectation is plotted in Fig.~\ref{fig:kamland} (right) as a function of $L/E_{\nu}$. The spectrum indicates almost two cycles of the periodic feature 
\textcolor{blue}{\st{as}} expected from neutrino oscillations, strongly disfavoring other explanations of the $\bar\nu_e$ disappearance.

The KamLAND results~\cite{Kamland03,Kamland05,Kamland08} are highly consistent with the solar neutrino experiments, and have pinned down the solar neutrino oscillation solution to the LMA region. When combined with the results from SNO, \textcolor{blue}{\st{it yields} they yield} the most precise measurements of $\tan^2\theta_{12} = 0.47^{+0.06}_{-0.05}$ and $\Delta m^2_{21} = 7.59^{+0.21}_{-0.21} \times 10^{-5}$ eV$^2$. \textcolor{red}{\st{It marks the beginning of a precision era in the neutrino oscillation experiments.}} 
\textcolor{blue}{\st{It shows} This is a}  \textcolor{red}{great example of the complementarity between different types of experiments. The SNO and KamLAND's first results came out approximately 18 months} \textcolor{blue}{\st{between} of} \textcolor{red}{ each other, with the solar experiment \st{being} more sensitive to the mixing angle $\theta_{12}$ and the reactor experiment to the mass-squared difference $\Delta m^2_{21}$. The observation of the same effect with two different sources on such different scales provides compelling evidence for neutrino oscillations.}


%%%%%%%%% Section: Theta_13 Experiments %%%%%%%%%
\section{Searching for the Smallest Oscillation Angle} 
\label{sec:theta13}
In contrast to the  Cabibbo-Kobayashi-Maskawa (CKM) matrix in quark mixing, where all three mixing angles are very small~\cite{PDG14}, the mixing angles in the neutrino mixing matrix appear to be large: $\theta_{23}$, measured by the atmospheric~\cite{Kajita} and long-baseline accelerator~\cite{Feldman} neutrino experiments, is \textcolor{red}{consistent with $45^\circ$ which corresponds to maximal mixing}; and $\theta_{12}$, measured by the solar neutrino experiments and KamLAND, is about $33^\circ$. It was therefore natural to expect 
\textcolor{blue}{that} the third mixing angle, $\theta_{13}$, \textcolor{blue}{\st{to}} might be of similar magnitude.

\begin{figure*}[tb]
  \centering
  \includegraphics[width=0.95\textwidth]{figs/Figure-4.pdf}
  \caption{ \label{fig:dayabay}{\bf The Daya Bay results~\cite{Zhang-Neutrino14}}. (left) The data points show the ratio of the detected to expected no-oscillation $\bar\nu_{e}$ signals at the 8 antineutrino detectors (ADs) located in three experimental halls as a function of 
  \textcolor{blue}{the} effective baseline. A $\sim$6\% signal deficit at the far hall relative to the near halls is observed. 
...
The importance of knowing the precise value of $\theta_{13}$ provoked a series of world-wide second-generation kilometer-baseline reactor experiments in the 21st century, including Double Chooz~\cite{DChooz} in France, RENO~\cite{Reno} in Korea and Daya Bay~\cite{Dayabay} in China, to push the sensitivity to $\theta_{13}$ 
\textcolor{blue}{\st{to much} considerably} below $10^\circ$. 
...
A common technology used in both the first and second generation experiments is the gadolinium-loaded liquid scintillator as 
\textcolor{blue}{the} $\bar\nu_{e}$ detection target. Gd has a high thermal neutron capture cross section. With $\sim$0.1\% gadolinium loading, the neutron capture time is reduced to $\sim$28 microseconds from $\sim$200 microseconds for the un-loaded scintillator (as 
\textcolor{blue}{\st{is}} used in KamLAND.) Furthermore, Gd deexcitation after the 
\textcolor{blue}{neutron} capture releases an 8-MeV gamma-ray cascade, which gives a delayed signal well above natural radioactivity (in contrast, 
neutron capture on \textcolor{blue}{a} proton releases a single 2.2-MeV $\gamma$). The accidental coincidence background is therefore drastically reduced.

\textcolor{blue}{ Addition of near detectors at baselines of a few hundred meters is the most significant improvement 
of the second-generation experiments over the previous ones.} 
As discussed in Section II, the uncertainty in predicting the reactor antineutrino flux is relatively large (2--5\%). 
This flux uncertainty, however, can be largely \textcolor{blue}{\st{canceled from a} eliminated by the} relative measurement between near and far detectors. 
The Double Chooz experiment expands CHOOZ by adding a near detector at a distance of $\sim$400 m. 
The installation of \textcolor{blue}{\st{the} that} near detector, however, was delayed due to civil construction. 
...
The two identical detectors were located at 290 and 1380 m, respectively, from the center of \textcolor{blue}{the} reactor array. 
...
This modular detector design \textcolor{blue}{\st{ further}} allows Daya Bay to largely remove the correlated detector systematics. 
Daya Bay started taking data in December 2011.

Compared to the first-generation experiments, the second-generation experiments have much larger 
\textcolor{blue}{ signal} statistics by utilizing higher power reactors and larger detectors. Among them, Daya Bay has the largest reactor power (17.4 GW$_{th}$) and target mass (80 tons at the far site,) as shown in Table~\ref{tab:theta13}. 
...
The signal rate at the far site shows \textcolor{blue}{\st {a clear} an obvious}
...
Although \textcolor{blue}{the} last known, the precision in $\theta_{13}$ measurement (6\%) is now the best among all three mixing angles.
...
The best-fit frequency of the oscillation yields $|\Delta{m}^2_{31}| = 2.47^{+0.11}_{-0.10} \times 10^{-3}$ eV$^2$ (assuming normal mass hierarchy), which is in good agreement and of comparable precision \textcolor{blue}{\st{with} compared to} the results from atmospheric and long-baseline accelerator neutrino experiments. 
...
\textcolor{blue}{to} measure the CP-violating phase in the leptonic sector.
...
The neutrino mass hierarchy (MH) is a problem of \textcolor{blue}{\st{the}} fundamental importance~\cite{MHwhitepaper}.
